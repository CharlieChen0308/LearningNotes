...
| # | 任務 | 狀態 |
|---|------|------|
| 4.1 | 更新 README.md — 新目錄完整索引 + 閱讀路線建議 | `[ ]` |
| 4.2 | 全局交叉引用檢查 — 確認所有篇章間連結正確 | `[ ]` |
| 4.3 | MarkdownReader 測試 — 開啟每個連結確認可跳轉 | `[ ]` |
| 4.4 | 刪除 `_archive/` 中的重複檔案（`_posts/` 與主目錄重複的） | `[ ]` |
| 4.5 | 最終 Git commit：`docs: LearningNotes v2 restructure complete` | `[ ]` |

**Phase 4 驗收 Checklist：**

- `[ ]` README.md 列出所有 ~50 篇文章，無損壞連結
- `[ ]` 從 README.md 點擊任意連結，MarkdownReader 可正確開啟
- `[ ]` `_archive/` 資料夾存在且包含所有舊文章
- `[ ]` 無任何文章引用 `_archive/` 內的檔案（單向：歸檔文章可引用新文章，反向不行）
- `[ ]` 新文章無外部圖片依賴（GitHub FigureBed 等）
