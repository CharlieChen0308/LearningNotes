...
description: 併發性上不去是因為當多個執行緒同時訪問一行資料時，產生了事務，因此產生寫鎖，當一個獲取了事務的執行緒把鎖釋放，另一個排隊執行緒才能拿到寫鎖，QPS(Query Per Second每秒查詢率)和事務執行的時間有密切關係，事務執行時間越短，併發性越高，這也是要將費時的 IO 操作移出事務的原因。
---
